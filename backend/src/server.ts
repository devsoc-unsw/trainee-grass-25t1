...
app.use(cors({
  origin: ["http://localhost:3001", "https://studysync.denzeliskandar.com"],
  credentials: true
}));
...
app.post('/auth/register', async (req: Request, res: Response) => {
  try {
    ...
    const { accessToken, refreshToken, userId, userName, userUsername } = await authRegister(name, email, password, username, institution);

    // Assign cookies
    res.cookie('accessToken', accessToken, { httpOnly: isProduction, path: "/", secure: isProduction, sameSite: isProduction ? "none" : "lax", domain: isProduction ? "studysync.denzeliskandar.com" : ".localhost", maxAge: 1800000 });
    res.cookie('refreshToken', refreshToken, { httpOnly: isProduction, path: "/", secure: isProduction, sameSite: isProduction ? "none" : "lax", domain: isProduction ? "studysync.denzeliskandar.com" : ".localhost", maxAge: 7776000000 });

    res.header('Access-Control-Allow-Credentials', 'true');

    res.status(200).json({ userId: userId, userName: userName, userUsername: userUsername });
  } catch (error: any) {
    console.error(error);
...
app.post('/auth/login', async (req: Request, res: Response) => {
  try {
    ...
    const { accessToken, refreshToken, userId, userName, userUsername } = await authLogin(email, password);
...
async function authenticateToken(req: Request, res: Response, next: NextFunction) {
  ...
  if (!accessToken && !refreshToken) res.status(401).json({ error: "No token provided." });
  ...
  try {
    ...
    if (!refreshToken) { res.status(401).json({ error: "No refresh token provided." }); }
